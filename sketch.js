...
//destroy zombie, player and bullets and display a message in gameState "bullet"
  ...
  textSize(50)
  fill("yellow")
  text("You ran out of bullets!!!",windowWidth/2-200,windowHeight/2)
  zombieGroup.destroyEach();
  player.destroy();
  bulletGroup.destroyEach();
...
function enemy(){
  if(frameCount%50===0){
    ...
    zombie = createSprite(1200,random(100,500),40,40)
...
function regeneration(){
  if(frameCount%160===0){
    ...
    //giving random x and y positions for zombie to appear
    regen = createSprite(1200,random(100,500),40,40)

    regen.addImage(regenImg)
    regen.scale = 0.15
    regen.velocityX = -3
    regen.debug= false
    regen.setCollider("rectangle",0,0,150,150)
   
    regen.lifetime = 400
   regenGroup.add(regen)
  }
